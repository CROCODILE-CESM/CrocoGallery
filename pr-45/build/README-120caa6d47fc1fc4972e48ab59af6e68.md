<div>
  <h1>CrocoGallery</h1>
  <p>
    Notebooks, tutorials, and showcases for the CROCODILE ecosystem — regional ocean modeling with MOM6 in CESM.
  </p>
</div>

<div class="croco-hub__grid">
  <a class="croco-hub-card croco-hub-card--featured" href="workshop_2026/index.md">
    <span class="croco-hub-card__badge">Sept 28 – Oct 3</span>
    <div class="croco-hub-card__icon">🐊</div>
    <h2 class="croco-hub-card__title">2026 CROCODILE Workshop</h2>
    <p class="croco-hub-card__desc">
      Agenda, practicums, and everything you need to have ready before day one.
    </p>
    <span class="croco-hub-card__cta">Agenda &amp; practicums →</span>
  </a>
  <a class="croco-hub-card" href="crocodash/index.md">
    <div class="croco-hub-card__icon">🐊</div>
    <h2 class="croco-hub-card__title">CrocoDash</h2>
    <p class="croco-hub-card__desc">
      Configure and deploy regional MOM6 ocean models. Grid generation, boundary conditions, forcing data, and coupling through the CESM — all in one Python toolkit.
    </p>
    <span class="croco-hub-card__cta">Tutorials, features, use cases →</span>
  </a>
  <a class="croco-hub-card" href="diagnostics/index.md">
    <div class="croco-hub-card__icon">📊</div>
    <h2 class="croco-hub-card__title">Diagnostics</h2>
    <p class="croco-hub-card__desc">
      mom6-tools powered diagnostics and analysis for regional MOM6 output. Time-series, surface fields, OBC diagnostics, and budget analysis.
    </p>
    <span class="croco-hub-card__cta">Explore diagnostics →</span>
  </a>
  <a class="croco-hub-card" href="model2obs/index.md">
    <div class="croco-hub-card__icon">🔬</div>
    <h2 class="croco-hub-card__title">model2obs</h2>
    <p class="croco-hub-card__desc">
      Model–observation comparisons via CrocoLake, a cloud-optimized Parquet observational database for fast access to data.
    </p>
    <span class="croco-hub-card__cta">Explore comparisons →</span>
  </a>
  <a class="croco-hub-card" href="dart/index.md">
    <div class="croco-hub-card__icon">🎯</div>
    <h2 class="croco-hub-card__title">DART Data Assimilation</h2>
    <p class="croco-hub-card__desc">
      Ensemble data assimilation for regional MOM6 in CESM using DART. Observation preparation, filter configuration, assimilation cycling, and diagnostics.
    </p>
    <span class="croco-hub-card__cta">Run DART with CESM-MOM6 →</span>
  </a>
</div>
